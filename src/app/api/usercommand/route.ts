...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ message: "Failed to get user commands" }))
...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch("http://localhost:3001/usercommand", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: request.headers.get("Authorization") || "",
    ...
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ message: "Failed to create user command" }))
      return NextResponse.json(
